...
global nCellsGh nChar nCombo steP implementBreakage implementGrowth thr p aChar M_NisTilde

steP = steP+1;
N_all = fixLowerThreshold(N_all,thr);
Nti_aChar = N_all(1:nCellsGh);
Nti_aChar = ghostToZero(Nti_aChar);
x1_aChar = N_all(nCellsGh+1);
w1_aChar = N_all(end);
...
if mod(steP,50) ==1 
% M_NisTilde = true;
 Marg_x1_aChar = computeMarg(Nti_aChar,M_NisTilde,w1_aChar,x2_widths,[],'x1'); % with this I obtain the marginal for x1 Marg_x1 (no tilde)
...
if implementBreakage
 [Bb,Vb,Db] = BreakagePBE(Nti_aChar,@(x2_all,aSp) BrRate(x2_all,[]),BrRateKer_b,BrRateKer_v,x2_all,[]);
