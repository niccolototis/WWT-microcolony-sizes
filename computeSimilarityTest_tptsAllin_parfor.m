...
function Dist = computeSimilarityTest_tptsAllin_parfor(aSt,Lambda_i)
global alpha
close all 
...
global kernelWidth plotsWhileStatTests alpha nDecimalsToIncludeNDFsampling folderOutput improvStr cycleModStr
...
if ~isfile([fold_S_compPD 'S_compPD_lambda_' labLambda improvStr cycleModStr '.mat'])
 ...
 for oneTpoint = 1:nTpoints
    ...
    parfor i = 1:length(oneNDF)
        fprintf('Sampling interval %g of the NDF at tpoint %g \n',i,oneTpoint)
 ...
 % save
 save([fold_S_compPD 'S_compPD_lambda_' labLambda improvStr cycleModStr '.mat'],'S','compPD')
 saveas(reestimNDFfig,[similarityFold 'reestimNDFfig_lambda_' labLambda improvStr cycleModStr '.fig'])
else
 load([fold_S_compPD 'S_compPD_lambda_' labLambda improvStr cycleModStr '.mat'],'S','compPD')
end
...
tmpFname = [similarityFold 'tmp_expSample_weights_' aW improvStr cycleModStr '.mat'];
if ~isfile(tmpFname)
...
dist_expSample.AD_all = sum(dist_expSample.AD);
dist_expSample.CVM_all = sum(dist_expSample.CVM);
...
while true
 ...
 AD_MC_allTps = zeros(mcReps,1);CVM_MC_allTps=zeros(mcReps,1);

 % Compute aStatMC
 parfor rep = 1:mcReps
    if mod(rep,100) ==0
        fprintf('Lambda %g Weight %s : MCrep = %g \n',Lambda_i,aW, rep)
    end
    ...
    for oneTpoint = 1:nTpoints
        ...
        % Store
        AD_MC_aTp_aRep = computeStat('AD',compCDF_at_randSample,n,myAddedWeights_aW);
        CVM_MC_aTp_aRep = computeStat('CVM',compCDF_at_randSample,n,myAddedWeights_aW);
        AD_MC_allTps(rep) = AD_MC_allTps(rep)+AD_MC_aTp_aRep;
        CVM_MC_allTps(rep) = CVM_MC_allTps(rep)+CVM_MC_aTp_aRep;
    end
 end
    
    critMC.AD = prctile(AD_MC_allTps, 100*(1-alpha));
    pMC.AD = sum(AD_MC_allTps > dist_expSample.AD_all)./mcReps;
    crit.AD = (mcRepsOld*crit.AD + mcReps*critMC.AD) / mcRepsTot;
    p.AD = (mcRepsOld*p.AD + mcReps*pMC.AD) / mcRepsTot;
    sepsq.AD = max(p.AD*(1-p.AD)/mcRepsTot, 1/mcRepsTot^2);

    critMC.CVM = prctile(CVM_MC_allTps, 100*(1-alpha));
    pMC.CVM = sum(CVM_MC_allTps > dist_expSample.CVM_all)./mcReps;
    crit.CVM = (mcRepsOld*crit.CVM + mcReps*critMC.CVM) / mcRepsTot;
    p.CVM = (mcRepsOld*p.CVM + mcReps*pMC.CVM) / mcRepsTot;
    sepsq.CVM = max(p.CVM*(1-p.CVM)/mcRepsTot, 1/mcRepsTot^2);

    sepsq_both = max(sepsq.AD,sepsq.CVM);
...
% Average
p_allTps.AD = p.AD;
crit_allTps.AD = crit.AD;

p_allTps.CVM = p.CVM;
crit_allTps.CVM = crit.CVM;
return
