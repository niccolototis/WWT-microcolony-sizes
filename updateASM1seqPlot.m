 ...
 switch P.parSetup
    ...
    case 'paramSweep'
        ...
        switch P.figure
            ...
            case 'flat'
                thisLev = data(1);
        %         thisColor = squeeze(P.colors(1,thisLev,:))'; % slicing the 3-D array at the first row. transposed cause it needs to be a row R,G,B        
                thisColor = P.allColors(thisLev,:);
                for oneRound = 1:rounds
                    ...
                    %         lgd = [];
                    %         for jj = 1:length(P.parToSweep) % for each of the parameters that I change
                    %             E = strcat(P.parToSweep{jj},'=');
